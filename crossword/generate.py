...
class CrosswordCreator():
    ...
    def assignment_complete(self, assignment):
        ...
        """
        for words in self.crossword.variables:
            if words not in assignment:
                return False
    ...
    def consistent(self, assignment):
        ...
        """
        set1 = set(assignment.values())
        if len(set1) != len(assignment):
            return False
                    ...
                    if overlap is not None:
                        ...
                        for neighbor_word in self.domains[neighbor]:
                            if word[i] != neighbor_word[j]:
                                count += 1
            penalty[word] = count
        return sorted(words, key=lambda w: penalty[w])      
